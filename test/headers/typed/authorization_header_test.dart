...
void main() {
  group('Given an Authorization header with validation', () {
    ...
    test('when a Authorization header with an invalid value is passed '
    ...
    group('and a Basic Authorization header', () {
      ...
      test('when a Basic token is passed then it should parse the credentials '
          'correctly', () async {
    ...
    group('and a Digest Authorization header', () {
      ...
      group('when a Digest token is passed with missing', () {
        test('"username" then the server responds with a bad request including '
            'a message that states the username is required', () async {
          const digestValue =
        ...
        test('"realm" then the server responds with a bad request including '
            'a message that states the realm is required', () async {
          const digestValue =
        ...
        test('"nonce" then the server responds with a bad request including '
            'a message that states the nonce is required', () async {
          const digestValue =
        ...
        test('"uri" then the server responds with a bad request including '
            'a message that states the uri is required', () async {
          const digestValue =
        ...
        test('"response" then the server responds with a bad request including '
            'a message that states the response is required', () async {
          const digestValue =
      ...
      test('when a Digest token is passed with all required parameters then it '
          'should parse the credentials correctly', () async {
        const digestValue =
            'username="user", realm="realm", nonce="nonce", uri="/", response="response"';
        final headers = await getServerRequestHeaders(
          server: server,
  ...
  parameterizedGroup(
    ...
    (final v) {
      ...
      singleTest(
        ...
        () => BasicAuthorizationHeader(username: v.$1, password: v.$2),
        matcher,
      );
    },
  );
